...
- `server.proxy`：配置代理，将`/api`开头的请求转发到服务消费模块
- `target`：服务消费模块的地址
...
**前置条件**：已启动Nacos服务端和服务提供/消费模块

**详细操作流程**：
1. 访问`http://localhost:8848/nacos`，使用默认用户名密码`nacos/nacos`登录
2. 点击左侧菜单的"服务管理"→"服务列表"
3. 查看是否有`service-provider`和`service-consumer`服务
4. 预期结果：两个服务都显示在服务列表中，健康状态为"健康"
...
5. 进入前端目录，执行`npm run build`构建前端静态资源
6. 使用Nginx或其他Web服务器部署前端静态资源
